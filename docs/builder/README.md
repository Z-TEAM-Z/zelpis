...
| `validateCustomHtml` | `false \| 'warn' \| 'strict'` | ❌ | HTML 校验级别，默认为 `'warn'`。详见 [HTML 校验](#html-校验) |
...
通过 `validateCustomHtml` 配置项控制校验行为：
...
export default defineConfig({
  zelpis: {
    ...
    validateCustomHtml: 'strict', // 有错误会终止构建
  },
})
```
...
export default defineConfig({
  zelpis: {
    validateCustomHtml: false, // 完全关闭校验
    entrys: [
      {
        basePath: '/',
        entryPath: './entry.ts',
...
- **时间紧急** - 可以设置 `validateCustomHtml: false` 暂时跳过校验
