...
| 项 | 要求 |
|----|------|
| Node.js | `>= 22.12.0`（见 `engines.node`） |
| pnpm | 与 `packageManager` 字段一致（如 `pnpm@10.17.x`），建议启用 Corepack：`corepack enable` |
...
同时请在你的项目中安装 Vite 以及对应框架插件，例如：

- Vue：`@vitejs/plugin-vue`
- React：`@vitejs/plugin-react`

（版本号按你项目与 Zelpis peer 要求选择。）
